...
#define w_cond_timedwait(x, y, z) ({ int error = pthread_cond_timedwait(x, y, z); if (error != 0 && error != ETIMEDOUT) merror_exit("At pthread_cond_timedwait(): %s", strerror(error)); error; })
